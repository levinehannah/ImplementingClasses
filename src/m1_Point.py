...
class Point(object):
    ...
    def move_to(self, x, y):
        self.dist = math.sqrt((x - self.ogx)**2 + (y - self.ogy)**2)
        self.displacement += math.sqrt((x - self.x)**2 + (y - self.y)**2)
        self.x = x
        self.y = y
        self.moves +=1
        return Point(self.x + self.moves, self.y + self.moves)
    ...
    def get_distance_from(self,point1):
        dist = math.sqrt((point1.x-self.x)**2 + (point1.y - self.y)**2)
        return dist
    ...
    def halfway_to(self, p2):
        point = Point(self.x + (p2.x - self.x)/2, self.y + (p2.y-self.y)/2)
        return point
